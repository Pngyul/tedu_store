$(function() {
	...
	$('.del_d').click(function() {
			var id=$(this).parent().siblings('.check').children('.each').children('input').val();
			//alert(id);
			console.log(id);
			$('.modal').fadeIn();
			$('.no').click(function(){
	...
	$(".foot_del").click(function() {
		...
		var paramsArray = str.toString().split(",");
		var data = ""; // 目标格式：ids=xx&ids=xx&ids=xx
		for (var i = 0; i < paramsArray.length; i++) {
			data += "&cart_ids=" + paramsArray[i];
		}
		if(str.length>0){
			$('.modal').fadeIn();
			$('.no').click(function(){
...
$('.foot_cash').click(function(){
	...
	console.log(str);
	if(str.length>0){
		var url = '../web/orderConfirm.jsp?itemIds='+str;
		 window.location.href = url;
	}else{
		$('.modalBalance').fadeIn();
		$('.close').click(function(){
